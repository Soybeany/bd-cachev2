...
class CacheNode<Param, Data> {
    ...
    public boolean dataCheck(DataContext<Param> context, IDataChecker<Param, Data> checker, IDatasource<Param, Data> datasource, boolean needStore) {
        return exeWithLock2(context, useLock -> {
            if (!useLock) {
                return false;
            }
            DataPack<Data> pack = getDataFromCurNodeOrNext(context, datasource, needStore);
            boolean needUpdate = onCheck(checker, pack, context);
            return isFromDatasource(pack) || needUpdate;
        });
    }
    ...
    private boolean onCheck(IDataChecker<Param, Data> checker, DataPack<Data> pack, DataContext<Param> context) {
    ...
    private <T> T exeWithLock2(DataContext<Param> context, Function<Boolean, T> callback) {
    ...
    private void exeWithLocks(Collection<String> keys, Consumer<Boolean> callback) {
        exeWithLock(() -> {
    ...
    private boolean isFromDatasource(DataPack<Data> pack) {
        return pack.provider instanceof IDatasource;
    }
